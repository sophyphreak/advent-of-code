...
const doTheThing = async (getArr, yRow) => {
  let arr = await getArr()
  const sensors = buildSensors(arr)
  const corners = buildCorners(sensors)
  let intervals = buildIntervals(corners, yRow)
  intervals = mergeIntervals(intervals)
  const count = getCount(intervals)
  return count
}
...
doTheThing(getInput, 2000000).then(console.log)
