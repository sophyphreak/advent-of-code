...
const countOctopuses = octopusMatrix =>
  octopusMatrix.reduce((count, row) => count + row.length, 0)
...
const incrementOctopuses = octopusMatrix => {
  ...
  const octopusCount = countOctopuses(octopusMatrix)
  if (enteredFlashQueue.size === octopusCount) {
    return { allFlashed: true, octopusMatrix }
  }
  return { allFlashed: false, octopusMatrix }
}
...
const doTheThing = async getArr => {
  ...
  while (true) {
    step++
    ;({ allFlashed, octopusMatrix } = incrementOctopuses(octopusMatrix))
    if (allFlashed) break
  }
  return step
}
