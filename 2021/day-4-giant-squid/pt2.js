...
const findWinnerBoards = ({ markedBoards, winnerBoards }) => {
  for (let index = 0; index < markedBoards.length; index++) {
    if (winnerBoards[index]) {
      continue
    }
    const markedBoard = markedBoards[index]
    const isWinner = checkIsWinner(markedBoard)
    if (isWinner) {
      winnerBoards[index] = true
    }
  }
  return winnerBoards
}

const getLastWinner = ({ picks, boards, markedBoards }) => {
  let winnerBoards = new Array(boards.length).fill(false)
  ...
  for (const pick of picks) {
    ...
    winnerBoards = findWinnerBoards({ markedBoards, winnerBoards })

    // if all boards are winners except one board
    if (winnerBoards.filter(isWinner => !isWinner).length === 1) {
      winnerBoards.forEach((isWinner, index) => {
        if (!isWinner) {
    ...
    if (winnerBoards.filter(isWinner => !isWinner).length === 0) {
      return {
        pick,
        board: boards[lastWinnerIndex],
        markedBoard: markedBoards[lastWinnerIndex],
      }
    }
  }
}
...
const doTheThing = async getArr => {
  ...
  const lastWinner = getLastWinner({ picks, boards, markedBoards })
  const finalScore = getFinalScore(lastWinner)
