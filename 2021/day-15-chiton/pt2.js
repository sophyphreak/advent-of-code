...
const buildBiggerChitronMatrix = chitonMatrix => {
  chitonMatrix = chitonMatrix.map(str => str.split("").map(str => +str))
  const biggerChitronMatrix = new Array(chitonMatrix.length * 5)
    .fill(null)
    .map(_ => new Array(chitonMatrix[0].length * 5).fill(-1))
  for (let i = 0; i < biggerChitronMatrix.length; i++) {
    for (let j = 0; j < biggerChitronMatrix[0].length; j++) {
      let value =
        chitonMatrix[i % chitonMatrix.length][j % chitonMatrix[0].length] +
        Math.floor(i / chitonMatrix.length) +
        Math.floor(j / chitonMatrix[0].length)
      if (value > 9) {
        value = value % 9
      }
...
const doTheThing = async (getArr, steps) => {
  ...
  const endPoint = `${biggerChitronMatrix.length - 1}_${
    biggerChitronMatrix[0].length - 1
  }`
